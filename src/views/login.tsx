...
export function Login() {
  ...
  return (
    <form onClick={() => clearErrors()} onSubmit={handleSubmit(onSubmit)}>
      ...
      <input aria-invalid={errors.password ? true : false} {...register("password")} type="password" placeholder='Password' />
      {errors.form && <span className='error'>{errors.form.message as unknown as string}</span>}
      <button aria-busy={mutation.isLoading ? true : false} disabled={mutation.isLoading} type="submit">Submit</button>
    </form>
  );
}